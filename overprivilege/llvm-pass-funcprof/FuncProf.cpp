...
namespace {
  struct FuncProfPass : public FunctionPass {
    static char ID;
    FuncProfPass() : FunctionPass(ID) {}
...
*/  }
  ...
  bool getFuncArg(Function& F) {
    ...
    for (Function::ArgumentListType::iterator arg = F.getArgumentList().begin(); arg != F.getArgumentList().end(); arg++) {
          ...
          if (targetMsg.find(typeSanitizer(type_str)) != targetMsg.end()) {// && arg_str.find("* "+var_str) != std::string::npos) {
              target_func = true;
...
//    opt -load ./FuncProf.so -defuse sum.bc > /dev/null
// See http://llvm.org/releases/3.4/docs/WritingAnLLVMPass.html#running-a-pass-with-opt for more info.
RegisterPass<FuncProfPass> X("funcprof", "FunctionProfile Pass");
