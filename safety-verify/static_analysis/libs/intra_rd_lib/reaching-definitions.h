...
//          errs() << "applyTransfer " << instr << "\n";
          ...
          for (DenseMap<Value*, int>::const_iterator prevDefIter = domainEntryToValueIdx.begin();
               ...
               ++prevDefIter) {
            Value* prevDefStr = valueToDefinitionVar(prevDefIter->first); //std::string prevDefStr = valueToDefinitionVarStr(prevDefIter->first);
            Value* currDefStr = valueToDefinitionVar(currDefIter->first); //std::string currDefStr = valueToDefinitionVarStr(currDefIter->first);
//            errs() << "prevDefStr: " << prevDefStr << ", currDefStr: " << currDefStr << "\n";
  ...
  void getOperandDefVals(Instruction* inst, std::map<Value*, std::vector<Value*> >& reaching_def_instr, std::map<Value*, BasicBlock*>& instr_bb_map, std::map<BasicBlock*, std::string>& bb_name_map, std::map<Value*, int>& instr_id_map) {
    for (int i = 0; i < inst->getNumOperands(); i++) {
       ...
       for (int j = 0; j < reaching_def_instr[inst].size(); j++) {
           if (valueToDefinitionVar(reaching_def_instr[inst][j]) == val)  
   ...
   if (F.isDeclaration()) 
    ...
    for (Function::iterator basicBlock = F.begin(); basicBlock != F.end(); ++basicBlock) { 
         ...
         for (BasicBlock::iterator instruction = basicBlock->begin(); instruction != basicBlock->end(); ++instruction) {
              unsigned op_code = instruction->getOpcode();
              bool is_void = instruction->getType()->isVoidTy();
              if (!is_void || op_code == Instruction::Store) {
                  domain.push_back(&*instruction);
              }
              //if ((!is_void || op_code == Instruction::Store) == valueToDefinitionStr(&*instruction).empty())
              //    errs() << "Found instr " << (!is_void || op_code == Instruction::Store) << " " << curr_str.empty() << " " << valueToStr(&*instruction) << "\n";
    ...
    errs() << "\n****************** REACHING DEFINITIONS OUTPUT FOR FUNCTION: " << func_name << " *****************\n";
...
//        if (instr.find("  call void @llvm.dbg") == 0) continue;
        ...
        for (defIter = dataFlowResult.domainEntryToValueIdx.begin(); defIter != dataFlowResult.domainEntryToValueIdx.end(); ++defIter) {
          Value* prevDefStr = valueToDefinitionVar(defIter->first); //std::string prevDefStr = valueToDefinitionVarStr(defIter->first);
          if (prevDefStr == currDefStr)
...
//          errs() << "Curr instr: " << bb_name_map[instr_bb_map[&*instruction]] << "," << instr_id_map[&*instruction] << "\n"; //valueToStr(&*instruction) << "\n";
